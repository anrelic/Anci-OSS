...
inline fun <S, R: Registration<R>> RegistrarAdapter(crossinline handler: (Registrar<S, R>) -> RegistrarHandler<S, R>) = object: RegistrarAdapter<S, R> {
...
inline fun <S, reified R: Registration<R>> Registrar<S, R>.RegistrarHandler(crossinline handler: Registrar<S, R>.(S) -> R) = object: RegistrarHandler<S, R> {
    override val type: Class<R> get() = R::class.java
